...
uint32_t HashTableDirectoryPage::GetLocalDepthMask(uint32_t bucket_idx) {
  return ~(~0U >> local_depths_[bucket_idx] << local_depths_[bucket_idx]);
}

uint32_t HashTableDirectoryPage::GetGlobalDepth() { return global_depth_; }

void HashTableDirectoryPage::Init(page_id_t directory_page_id, page_id_t bucket_page_id) {
  page_id_ = directory_page_id;
  global_depth_ = 0;
...
uint32_t HashTableDirectoryPage::GetGlobalDepthMask() { return ~(~0U >> global_depth_ << global_depth_); }

void HashTableDirectoryPage::IncrGlobalDepth() {
  uint32_t previous_global_depth = global_depth_;
  uint32_t previous_size = 1 << previous_global_depth;
  global_depth_++;
  uint32_t size = 1 << global_depth_;
  for (uint32_t i = previous_size; i < size; i++) {
    uint32_t split_image_idx = i ^ (1 << previous_global_depth);
    local_depths_[i] = local_depths_[split_image_idx];
    bucket_page_ids_[i] = bucket_page_ids_[split_image_idx];
  }
}
...
uint32_t HashTableDirectoryPage::GetLocalHighBit(uint32_t bucket_idx) {
  ...
  return (bucket_idx >> (local_depths_[bucket_idx] - 1)) & 1;
}
